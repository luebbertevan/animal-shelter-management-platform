# CO Kitty Coalition Brand Colors & Design System

## Overview

This document defines the color palette and usage guidelines for the CO Kitty Coalition animal shelter management platform. Colors are organized by their role in the logo and their intended UI purpose.
...
1. **Treat logo groupings as "roles," not just colors** — Each color group has a specific purpose based on its role in the logo.

2. **Maintain hierarchy** — Use structure colors (cyan family) for layout, brand voice (green) for communication, and action colors (orange) sparingly for emphasis.

3. **Preserve contrast** — Ensure text remains readable against backgrounds, especially when using the very dark gray (`#1A1A1A`) on light backgrounds.
