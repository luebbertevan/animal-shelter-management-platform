-- Migration: Update storage SELECT policy to allow viewing group photos
-- The current policy only allows viewing animal photos and conversation photos
-- This adds support for viewing group photos from the user's organization

-- Drop the existing SELECT policy
DROP POLICY IF EXISTS "Users can view photos from accessible conversations" ON storage.objects;

-- Recreate the policy to allow viewing conversation photos, animal photos, and group photos
...
USING (
  ...
  (
    ...
    OR
    -- Allow viewing group photos from user's organization
    (
      split_part(name, '/', 1) = public.get_user_organization_id()::text
      AND split_part(name, '/', 2) = 'groups'
    )
  )
);
