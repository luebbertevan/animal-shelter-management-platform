-- Foster request coordinator RPCs (approve/deny)
-- These functions run as SECURITY DEFINER to safely perform multi-table updates
-- (update foster_requests + animals/groups + auto-deny other pending requests)
-- while enforcing that the caller is a coordinator in the organization.
